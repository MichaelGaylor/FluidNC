...
public:
    ...
    return ConnectSTA2AP(SSID, password);   // note: signature takes SSID/pass
        ...
        static Error listAPs(const char* parameter, AuthenticationLevel auth_level, Channel& out) {  // ESP410
            ...
            // is a good chance that scan information is available.
            int n;
            while (true) {
                n = WiFi.scanComplete();
...
// === Helpers (file-scope in namespace WebUI): password state + set with confirmation ===
static Error showPasswordState(const char* parameter, AuthenticationLevel auth_level, Channel& out) {
    auto has = [](const char* s) { return (s && *s); };
    const char* sp = _sta_password->get();
    const char* ap = _ap_password->get();
    {
        std::string msg = std::string("Sta/Password set: ") + (has(sp) ? "yes" : "no")
                          + " len=" + std::to_string(has(sp) ? (int)strlen(sp) : 0);
        log_msg_to(out, msg.c_str());
    }
    {
        std::string msg = std::string("AP/Password set: ") + (has(ap) ? "yes" : "no")
                          + " len=" + std::to_string(has(ap) ? (int)strlen(ap) : 0);
        log_msg_to(out, msg.c_str());
    }
    return Error::Ok;
}
...
// WARNING: Diagnostic only — echoes plaintext STA password on request
static Error showStaPasswordPlain(const char* parameter, AuthenticationLevel auth_level, Channel& out) {
    const char* p = _sta_password->get();
    if (!p) p = "";
    log_msg_to(out, std::string("STA PASSWORD (PLAIN): ") + p);
    return Error::Ok;
}
